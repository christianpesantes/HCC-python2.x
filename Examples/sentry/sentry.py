...
class Gun(object):
    ...
    def __init__(self,texture,screenSize,rangeArea):
        self.sprite = pygame.sprite.Sprite()
        self.sprite.image = texture
        self.sprite.rect = self.sprite.image.get_rect()
...
class Sentry(object):
    ...
    def setup(self):
        ...
        self.gun = Gun(self.textures["billblaster"],self.screenSize,250)
        return
    ...
    def update(self):
        ...
        if not self.pending2exit:
            ...
            if not self.showFire:
                self.gun.update(self.timePassed)
                self.updateBombs()
                self.updateBullets()
    ...
    def updateBombs(self):
        for i in xrange(len(self.bombs)):
            ...
            if self.bombs[i].magnitude <= self.gun.rangeArea and self.gun.able2shoot:
                ...
                self.gun.angle = (math.acos(self.gun.trackingVect[0]/self.gun.trackingMagnitude))*180.0/math.pi
                if self.gun.trackingVect[1]>0:
                    self.gun.angle = 360-self.gun.angle
                self.gun.sprite.image = pygame.transform.rotate(self.textures["billblaster"],self.gun.angle)
                self.gun.sprite.rect = self.gun.sprite.image.get_rect()
                self.gun.sprite.rect.left = self.screenSize[0]/2-self.gun.sprite.rect[2]/2
                self.gun.sprite.rect.top = self.screenSize[1]/2-self.gun.sprite.rect[3]/2
                if self.gun.able2shoot:
                    self.gun.able2shoot = False
